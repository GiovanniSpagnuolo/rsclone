...
export const MaterialEditor = ({ onClose }: MaterialEditorProps) => {
  ...
  const fetchDatabaseTruth = async () => {
    try {
      ...
      localStorage.setItem('cache_materials', JSON.stringify(data.materials));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  return (
    <div style={{
      ...
      width: '600px', height: '450px', backgroundColor: 'rgba(20,20,20,0.95)', 
      ...
      <div style={{ width: '200px', borderRight: '1px solid #555', padding: '10px', overflowY: 'auto' }}>
        <div style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
          <button onClick={() => setActiveMat({ id: materials.length + 1, name: 'New Material', color: '#ffffff', isWalkable: true })} style={{ flex: 1, padding: '8px', background: '#4CAF50', color: 'white', border: 'none', cursor: 'pointer' }}>+ New</button>
        ...
        {materials.map(mat => (
          <div key={mat.id} onClick={() => setActiveMat(mat)} style={{ padding: '8px', cursor: 'pointer', background: activeMat.id === mat.id ? '#333' : 'transparent', borderBottom: '1px solid #444', display: 'flex', alignItems: 'center', gap: '8px' }}>
